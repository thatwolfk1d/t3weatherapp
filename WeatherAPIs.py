...
def get_current_forecast(x, y):
    ...
    for key in current_weather_data:
        if key == "weather":
            ...
            mydict = {"Header":key,key:current_weather_data[key]}
            mylist.append(mydict)
...
def get_hourly_forecast(x, y):
    ...
    for key in hourly_weather_data:
        if key == "list":
            for item in hourly_weather_data[key]:
                mydict = {}
                mydict["Header"]=list(item.values())[-1]
                mydict.update(item)
                mylist.append(mydict)
        elif type(hourly_weather_data[key]) == dict:
            mydict = {}
            mydict["Header"]=key
            mydict.update(hourly_weather_data[key])
            mylist.append(mydict)
